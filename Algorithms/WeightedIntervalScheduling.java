...
public class WeightedIntervalScheduling {
    ...
    private ArrayList<Integer> includedJobs = new ArrayList<Integer>(); // holds jobs in optimal solution. The id's are
                                                                        // id's of the sorted jobs, so must be converted
                                                                        // to the original ID's by getJobInfo
    ...
    public void calcSchedule(int[][] inputJobs) {
        ...
        for (int i = 1; i < jobs.length; i++) {
            memo[i] = Math.max(jobs[i][3] + memo[latestCompatible(i)], memo[i - 1]); // add max value if job is included
            // or if it's not included
        }
        ...
        for (int i = includedJobs.size() - 1; i >= 0; i--) { // Loop backwards to display jobs in increasing order of
                                                             // their ID's
            System.out.println(getJobInfo(includedJobs.get(i)));
        }
    }

    // Find the index of the job finishing before job i starts (uses jobs[][] array
    ...
    private int latestCompatible(int i) {
        ...
        return 0; // No compatible job was found. Return 0 so that value of placeholder job in
                  // jobs[0] can be used
    }

    // Iterative version of the recursive code to retrace & find the optimal
    // solution
    public void findSolutionIterative(int j) {
        while (j > 0) { // Stops when j==0
            ...
            if (jobs[j][3] + memo[compatibleIndex] > memo[j - 1]) { // Case where job j was included (from optimal
                                                                    // substructure)
                includedJobs.add(j); // add job index to solution
                j = compatibleIndex; // update j to the next job to consider
            } else { // case where job j was NOT included, remove job j from the possible jobs in the
                     // solution & look at jobs 1 to (j-1)
                j = j - 1;
            }
        }
    }
    ...
    private void findSolutionRecursive(int j) {
        if (j == 0) { // base case
            ...
            if (jobs[j][3] + memo[compatibleIndex] > memo[j - 1]) { // Case where job j was included (from optimal
                                                                    // substructure)
                includedJobs.add(j); // add job index to solution
                findSolutionRecursive(compatibleIndex); // recursively find remaining jobs starting the the latest
                                                        // compatible job
            } else { // case where job j was NOT included, remove job j from the possible jobs in the
                     // solution
    ...
    private String getJobInfo(int jobIndex) {
        return "Job " + jobs[jobIndex][0] + ":  Time (" + jobs[jobIndex][1] + "-" + jobs[jobIndex][2] + ") Value="
                + jobs[jobIndex][3];
    }
